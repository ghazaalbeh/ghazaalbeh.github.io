...
// Crop Images in Image Slider
 ...
 // adds .naturalWidth() and .naturalHeight() methods to jQuery for retrieving a normalized naturalWidth and naturalHeight.
    ...
    var
        carousels_on_page = $('.carousel-inner2').length,
        carouselWidth,
        carouselHeight,
        ratio,
        imgWidth,
    ...
    for(var i = 1; i <= carousels_on_page; i++){
        $('.carousel-inner2').eq(i-1).addClass('id'+i);
    };

    function imageSize() {
        setTimeout(function () {
            for(var i = 1; i <= carousels_on_page; i++){
                carouselWidth = $('.carousel-inner2.id'+i+' .item').width();
                carouselHeight = $('.carousel-inner2.id'+i+' .item').height();
                ratio = carouselWidth/carouselHeight;

                images_in_carousel = $('.carousel-inner2.id'+i+' .item img').length;

                for(var j = 1; j <= images_in_carousel; j++){
                    this_image = $('.carousel-inner2.id'+i+' .item img').eq(j-1);
                    imgWidth = this_image.naturalWidth();
                    imgHeight = this_image.naturalHeight();
                    imgRatio = imgWidth/imgHeight;
    ...
    imageSize();
    $(window).resize(function() {
        $('.carousel-indicators2 .first').click();
        imageSize();
    });
